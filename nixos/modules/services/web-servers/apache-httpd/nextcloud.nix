...
let
  ...
  nextcloudConfig = pkgs.writeText "config.php"
    ''
      ...
      */
      "logfile" => "${config.dataDir}/nextcloud.log",
...
rec {
  ...
  options = {
    ...
    package = mkOption {
      ...
      example = literalExample "pkgs.nextcloud";
      description = ''
    ...
    urlPrefix = mkOption {
      ...
      example = "/nextcloud";
      description = ''
        The URL prefix under which the nextcloud service appears.
      '';
    };
    ...
    id = mkOption {
      default = "main";
      description = ''
        A unique identifier necessary to keep multiple nextcloud server
  ...
  startupScript = pkgs.writeScript "nextcloud_startup.sh" ''
    ...
    ${pkgs.sudo}/bin/sudo -u wwwrun touch ${config.dataDir}/storage/.ocdata 
    ln -s ${config.package}/occ ${config.dataDir}/occ 
    ${pkgs.sudo}/bin/sudo -u wwwrun ${php}/bin/php ${config.package}/occ --help >> ${config.dataDir}/upgrade.log || true
